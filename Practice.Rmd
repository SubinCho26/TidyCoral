...
head(buoy)
```
get ride of double row title columns, sep is string to insert between input vectors 
- can't seem to modify column names through linking with data frame 
-i think the filtering conditions is not filtering any rows but idk why. 
-might be from mistake in previous code above 

```{r}
head(buoy) <- str_c(head(buoy),
                     buoy %>% filter ('#YY' == "#yr"), 
                     sep = "_")
names(buoy)
```
...
names(buoy)
```
cleaning up hashtags and slashses
```{r}
